...
public class TeacherService implements ITeacherService {
 ...
 @Override
    public long submit(String UID,TeacherChangeRequest r)throws SQLException{Teacher t=teachers.findByUID(UID);if(t==null)throw new IllegalArgumentException("当前用户没有教师档案");if(r==null||r.getItems()==null||r.getItems().isEmpty())throw new IllegalArgumentException("修改项不能为空");if(requests.findPendingByTeacherId(t.getTeacherId())!=null)throw new IllegalStateException("已有待审核申请");Set<String> seen=new HashSet<>();for(TeacherChangeItem i:r.getItems()){if(!EDITABLE.contains(i.getFieldName())||!seen.add(i.getFieldName()))throw new SecurityException("无权修改字段: "+i.getFieldName());if(Set.of("department","education","employmentStartDate").contains(i.getFieldName())&&(i.getNewValue()==null||i.getNewValue().isBlank()))throw new IllegalArgumentException(i.getFieldName()+"不能为空");}for(TeacherChangeItem item:r.getItems()) {
 ...
 item.setOldValue(actual);teachers.canonicalValue(item.getFieldName(),item.getNewValue());
 ...
 @Override
    public boolean updateByAdmin(Teacher t,Teacher original)throws SQLException{
		...
		validateRequiredJob(t);boolean ok=teachers.updateIfUnchanged(t,original);if(ok){new UserDAO().syncUserInfo(t.getUID()!=null?t.getUID():t.getTeacherId());}return ok;
	}
