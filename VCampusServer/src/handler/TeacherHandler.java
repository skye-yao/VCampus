...
public class TeacherHandler {private final ITeacherService service=new TeacherService();private final Gson gson=new Gson();
 ...
 case TEACHER_ADMIN_UPDATE->{needAdmin(admin);r.putData("updated",service.updateByAdmin(value(q,"teacher",Teacher.class),value(q,"original",Teacher.class)));}
 ...
 private String mutationKey(Message q,UserSession user,boolean admin)throws Exception {
  return switch(q.getType()) {
   case TEACHER_ADMIN_UPDATE -> {needAdmin(admin);yield "TEACHER:"+value(q,"teacher",Teacher.class).getTeacherId();}
   case TEACHER_REVIEW -> {needAdmin(admin);yield "TEACHER_CHANGE_REQUEST:"+value(q,"review",TeacherReviewVO.class).getRequestId();}
   case TEACHER_CHANGE_SUBMIT,TEACHER_WORK_EXPERIENCE_ADD,TEACHER_WORK_EXPERIENCE_UPDATE,TEACHER_WORK_EXPERIENCE_DELETE,
        TEACHER_FAMILY_MEMBER_ADD,TEACHER_FAMILY_MEMBER_UPDATE,TEACHER_FAMILY_MEMBER_DELETE -> "TEACHER:"+service.teacherIdForUser(user.getUsername());
   default -> null;
  };
 }
 private void needAdmin(boolean b){if(!b)throw new SecurityException("仅管理员可操作");} private String string(Message m,String k){Object v=m.getData().get(k);if(v==null)throw new IllegalArgumentException("缺少参数: "+k);return String.valueOf(v);} private long number(Message m,String k){
  // Gson 将 Message.data 中的 JSON 数字解码为 Double，例如申请编号 4 会成为 4.0。
  String value=string(m,k);
  try{return new java.math.BigDecimal(value).longValueExact();}
  catch(NumberFormatException|ArithmeticException e){throw new IllegalArgumentException("参数必须为有效整数: "+k);}
 }
