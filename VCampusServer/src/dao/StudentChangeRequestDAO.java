...
@SuppressWarnings( {
    ...
    public long insert(Connection c,StudentChangeRequest r)throws SQLException {
        try(PreparedStatement p=c.prepareStatement("INSERT INTO tblStudentChangeRequest(studentId,status,submitTime)VALUES(?,'PENDING',CURRENT_TIMESTAMP)",Statement.RETURN_GENERATED_KEYS)) {
            p.setString(1,r.getStudentId());
            p.executeUpdate();
    ...
    public StudentChangeRequest findByIdForUpdate(Connection c,long id)throws SQLException {
        return findById(c,id,true);
    }
    private StudentChangeRequest findById(Connection c,long id,boolean lock)throws SQLException {
        try(PreparedStatement p=c.prepareStatement("SELECT * FROM tblStudentChangeRequest WHERE requestId=?"+(lock?" FOR UPDATE":""))) {
            p.setLong(1,id);
            try(ResultSet r=p.executeQuery()) {
                if(!r.next())return null;
                StudentChangeRequest x=map(r);
    ...
    public List<StudentChangeRequest> findAll()throws SQLException {
        return list("SELECT * FROM tblStudentChangeRequest ORDER BY submitTime DESC",null);
    }
    ...
    public StudentChangeRequest findPendingByStudentId(Connection c,String id)throws SQLException {
        try(PreparedStatement p=c.prepareStatement("SELECT * FROM tblStudentChangeRequest WHERE studentId=? AND status='PENDING' ORDER BY submitTime DESC LIMIT 1")) {
            p.setString(1,id);
            try(ResultSet r=p.executeQuery()) {
                if(!r.next())return null;
                StudentChangeRequest x=map(r);
    ...
    public boolean review(Connection c,long id,StudentChangeStatus s,String who,String note)throws SQLException {
        try(PreparedStatement p=c.prepareStatement("UPDATE tblStudentChangeRequest SET status=?,reviewerId=?,reviewTime=CURRENT_TIMESTAMP,reviewRemark=? WHERE requestId=? AND status='PENDING'")) {
    ...
    public boolean cancel(long id,String sid)throws SQLException {
        try(Connection c=DBUtil.getConnection();PreparedStatement p=c.prepareStatement("UPDATE tblStudentChangeRequest SET status='CANCELLED' WHERE requestId=? AND studentId=? AND status='PENDING'")) {
            p.setLong(1,id);
            p.setString(2,sid);
            return p.executeUpdate()==1;
        }
    }
    private static StudentChangeRequest map(ResultSet r)throws SQLException {
        StudentChangeRequest x=new StudentChangeRequest();
        x.setRequestId(r.getLong("requestId"));
        x.setStudentId(r.getString("studentId"));
