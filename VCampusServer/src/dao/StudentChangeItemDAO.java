...
public class StudentChangeItemDAO {
    ...
    public List<StudentChangeItem> findByRequestId(Connection c,long id)throws SQLException {
        List<StudentChangeItem>o=new ArrayList<>();
        try(PreparedStatement p=c.prepareStatement("SELECT * FROM tblStudentChangeItem WHERE requestId=? ORDER BY itemId")) {
            p.setLong(1,id);
            try(ResultSet r=p.executeQuery()) {
                while(r.next()) {
                    StudentChangeItem x=new StudentChangeItem();
