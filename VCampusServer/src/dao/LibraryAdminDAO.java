    ...
    public List<Map<String,String>> findRecords(String kind) throws SQLException {
        ...
        if ("borrow".equals(kind) || "returns".equals(kind)) {
            sql="SELECT "+person+book+"r.borrowTime AS '借阅时间',r.dueTime AS '应还时间',r.returnTime AS '归还时间',"+
                "CASE WHEN r.status=3 THEN '遗失已赔偿' WHEN r.returnTime IS NULL AND EXISTS(SELECT 1 FROM tblLossRecord l WHERE l.userid=r.userid AND l.bookid=r.bookid AND l.status=0) "+
                "THEN CASE WHEN r.status=2 THEN '挂失中 / 逾期' ELSE '挂失中' END "+
        ...
        try(Connection conn=connections.open(); PreparedStatement stmt=conn.prepareStatement(sql); ResultSet rows=stmt.executeQuery()) {
