...
压缩内容的上限由**本仓库自己的代码**判定，不靠 POI 的默认值：`TeacherSpreadsheetService`
在打开工作簿之前先量文件本身（≤ 5 MiB，与文件票据的上限同一个数）并累加压缩包中央目录声明的
解压后总大小（≤ 64 MiB），任一项越界都在解压器启动之前失败。为什么不直接交给 POI：
`ZipSecureFile` 的默认最小压缩比是 0.01，等于允许膨胀到原文件的 100 倍（5 MiB → 约 500 MiB）
才拦下，而且只对真正被读到的条目生效——包里无人读取的巨型条目可以一路留到解析结束。
POI 的 `ZipSecureFile` 默认值继续保留在下一层：中央目录可以谎报大小，流式读取时的那道防线
（压缩比与单项大小）正好补上这一层，两者互补，无需为本项目改 POI 的全局静态配置。
