...
public class LibraryCirculationIntegrationTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try(Connection c=DBUtil.getConnection()) {
            ...
            seed(c);sql(c,"UPDATE tblReservation SET userid='admin'");c.commit();
            denied(()->dao.lend(1));
            equal(c,"SELECT COUNT(*) FROM tblBorrowRecord","0");
            equal(c,"SELECT status FROM tblBook","2");
            seed(c);dao.lend(1);check(books.changeLoss("reader",1,true),"report loss");
            LossRecordDAO notices = new LossRecordDAO();
            check(notices.findPublicNotices(c).size()==1,"loss notice visible");
            check(dao.recoverLostNotice(1),"delete notice and recover book");
            check(notices.findPublicNotices(c).isEmpty(),"recovered notice removed");
            check(!dao.recoverLostNotice(1),"already recovered notice");
            equal(c,"SELECT status FROM tblBook","0");
            equal(c,"SELECT COUNT(*) FROM tblLossRecord WHERE status=0","0");
            equal(c,"SELECT COUNT(*) FROM tblBorrowRecord WHERE status=1 AND returnTime IS NOT NULL","1");
            equal(c,"SELECT lossAmount FROM tblFineRecord","0.00");
            check(books.reserveAvailableBook("reader",1),"recovered book can be reserved");
            dao.lend(Integer.parseInt(value(c,"SELECT MAX(id) FROM tblReservation")));
            check(books.changeLoss("reader",1,true),"new loss");
            check(notices.findPublicNotices(c).size()==1,"new loss publishes new notice");
            int paidFine = Integer.parseInt(value(c,"SELECT MAX(id) FROM tblFineRecord"));
            dao.pay("reader",paidFine,"123456",money("50"));
            check(dao.recoverLostNotice(1),"recover compensated book via notice");
            equal(c,"SELECT status FROM tblBook","0");
            equal(c,"SELECT paidAmount FROM tblFineRecord WHERE id="+paidFine,"50.00");
            equal(c,"SELECT refundedAmount FROM tblFineRecord WHERE id="+paidFine,"0.00");
            equal(c,"SELECT balance FROM tbl_bank_account WHERE user_id='reader'","950.00");
            seed(c);
